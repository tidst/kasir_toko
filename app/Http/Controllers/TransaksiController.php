...
class TransaksiController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($search) $penjualan->appends(['search' => $search]);
    ...
    public function store(Request $request)
    {
        ...
        foreach ($allItems as $key => $value) {
            ...
            $produk = Produk::query()->where('kode_produk', $item->id)->orderBy('id', 'desc')->first();
            Log::info($produk);
    ...
    {
        $detilPenjualans = DetilPenjualan::query()->where('penjualan_id', $transaksi->id)->orderBy('id', 'desc')->get();
        foreach ($detilPenjualans as $detil) {
            $product = Produk::find($detil->produk_id);
            if ($product) {
                $product->update([
                    'stok' => $product->stok + $detil->jumlah,
                ]);
            }
        }
